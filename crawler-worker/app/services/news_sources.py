...
from app.crawlers.reddit_stack import search_reddit_stack_posts
...
def collect_korean_news_from_sources(category: str, limit: int = 24) -> list[dict]:
    ...
    for source_name in source_order:
        ...
        if category == "KR_STACK" and source_name in {"velog", "reddit"} and collected:
            return _dedupe_articles(collected)[:limit]
...
def _collect_korean_source(source_name: str, category: str, limit: int) -> list[dict]:
    ...
    if source_name == "reddit":
        return search_reddit_stack_posts(limit=limit)
    if source_name == "google":
