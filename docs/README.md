...
| [Book Memo K3s 준비·전환](operations-reference.md#book-memo-k3s-전환-승인검증) | 사전 승인 후 [이미지 build](../infra/k8s/tools/k3s-app-image-build.sh)·[import](../infra/k8s/tools/k3s-app-image-import.sh), [매니페스트](../infra/k8s/apps/book-memo.yaml), `--check`·`--prepare`·`--go`·`--rollback` 절차 확인 |
