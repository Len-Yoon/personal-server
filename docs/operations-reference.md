...
`car-care-worker`는 정비 상태용 `data/car-care`와 OAuth token/state 전용 Docker 관리 볼륨을 분리해 사용함. Telegram Bot long polling으로 명령과 알림을 처리하며, `car.len.pe.kr`은 Hyundai OAuth callback과 `/health`만 처리하도록 Cloudflare Tunnel에서 고정 Docker 주소 `172.30.0.10:8015`로 전달함. Caddy에는 이 호스트를 추가하지 않음.
