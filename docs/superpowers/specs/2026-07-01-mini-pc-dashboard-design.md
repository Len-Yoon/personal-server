...
The first version adds a lightweight `system-agent` service, a Windows startup bootstrap script, portal dashboard summaries, demo-mode masking, global search entry points, and README/operator documentation updates.

## Architecture

`system-agent` is a small FastAPI service that exposes `/health` and `/metrics`. It reads Docker/container status from a conservative local model, host metrics from `data/system/host-metrics.json` when present, backup status from `data/backups`, and file usage from `data/files`. It never needs write access outside the shared `data/` tree.

Windows startup automation uses a PowerShell bootstrap script that starts the Docker stack and Cloudflare Tunnel after boot. Host metrics stay optional; if the JSON is missing or stale, the agent returns a warning while keeping the portal usable.
...
- README and N100 docs: describe the new services, environment variables, and Windows startup entry setup.

## Data Flow

2. The startup bootstrap launches the Docker stack and Cloudflare Tunnel after boot.
