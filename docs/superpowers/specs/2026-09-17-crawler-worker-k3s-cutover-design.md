...
| 목적 | Docker Compose `crawler-worker`를 K3s 단일 writer로 전환하면서 뉴스 수집, SQLite 상태, metrics, `NewsCollectionStale`, Telegram 경계를 보존함 |
| 비고 | 실제 Docker 중지·PVC 복사·Tunnel/Caddy 전환은 테스트·독립 검토·별도 운영 승인 후에만 수행함 |

## 2. 핵심 요약

뉴스 수집기를 Cloudflare Tunnel → Caddy → K3s Service → Pod → PVC 경로로 전환함. 실제 전환 전까지 Docker만 수집 상태와 SQLite DB를 쓰며, 전환 중 scheduler를 포함한 Docker writer를 중지한 뒤 전체 데이터 디렉터리를 빈 PVC에 한 번만 복사함. K3s Pod가 수집을 시작한 뒤 Docker를 동시에 기동하지 않음.
...
- runtime state는 K3s Deployment·Service Endpoint·PVC Bound와 Docker writer 중지를 검증한 뒤에만 `crawler-worker=k3s`로 갱신함.

### 4.2 데이터와 scheduler 전환

1. Docker health, bind mount, SQLite `news_summaries.sqlite3` layout, K3s writer 부재를 확인함.
2. Docker crawler를 중지해 scheduler와 HTTP writer를 동시에 멈춤.
3. 빈 PVC에 `data/crawler-worker` 전체를 복사함. SQLite DB, 수집 상태 파일, 알림 중복 방지 상태를 함께 보존함.
4. 원본·PVC에서 SQLite `quick_check`와 전체 디렉터리 SHA-256 digest를 비교함.
5. 일치할 때만 K3s replica를 1로 확장함. 실패하면 K3s writer를 중지하고 원본 Docker만 복구함.
...
| 3 | 유지보수 전환 | 중지 | 없음 | 일시 중단 가능 | 원본 SQLite·상태 파일 검증 |
| 4 | 데이터 복사·K3s 시작 | 중지 | 실행 | 일시 중단 | digest 일치, Pod Ready |
| 5 | runtime state·Caddy·metrics 전환 | 중지 | 실행 | K3s | K3s Service metrics·Caddy 내부 health 성공 |
...
| 데이터 경로 | 실제 bind mount·SQLite 외 상태 파일·데이터 크기 | 복사 범위와 digest 기준 확정 필요 |
