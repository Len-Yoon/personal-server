...
| 구분 | 주기 | 유지 이유 | 중복 실행 정책 |
|---|---:|---|---|
| 코드 변경 검증 | 변경 시점 | 변경으로 인한 회귀를 병합 전 발견함 | 월간 감사의 대체 대상이 아님 |
| 외부 공개 상태 감시 | 약 5분 | 서버와 분리된 외부 경로에서 장애·복구를 조기에 감지함 | 월간 감사와 통합하지 않음 |
| Portal PVC 백업·복원 검증 | 매일 | 최신 복구 증적을 유지하고 만료·실패를 조기에 발견함 | 월간 감사는 증적만 읽기 확인함 |
| 내부 SRE 통합 감사 | 매월 1일 | 운영 상태를 한 번에 검토하고 결과를 relay로 전달함 | `monthly-sre-audit` 하나만 활성화함 |

보존된 분기·validation CronJob은 rollback·검증 이력용으로 suspended 상태를 유지함. 같은 내부 검증을 다른 활성 CronJob으로 중복 실행하지 않음.
...
5. 월간 통합 감사와 기존 분기 감사가 동시에 활성화되지 않도록 설치·상태·문서 계약을 갱신함. 일일 백업과 외부 공개 감시는 각자의 조기 감지 목적 때문에 계속 독립 실행함.
