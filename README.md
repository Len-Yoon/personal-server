...
| 항목 | 내용 |
|---|---|
| 로컬 변경 하니스 | 변경 범위와 검증 증적을 로컬에서 점검하는 변경 하니스 제공 |
| `--agent-context` | 에이전트 작업에 필요한 변경 컨텍스트를 명시적으로 생성하는 옵션 |
| 입력 증적 측정 | 6개 fixture 기준 입력 증적 바이트가 79.5% 감소함 |
| 측정 한계 | 실제 모델 토큰 감소량은 별도 측정 필요 |
