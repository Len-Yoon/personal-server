...
- `data/system/host-metrics.json`: optional Windows host snapshot that `system-agent` reads when available
...
`DELETE_PASSWORD`, `FILE_MANAGER_PASSWORD`, `ALADIN_TTB_KEY`는 `.env`에만 저장하고 README, 로그, 이슈, 커밋 메시지에 남기지 않습니다.
`OBSIDIAN_VAULT_PATH`는 기본적으로 프로젝트 기준 상대경로 `./data/obsidian/vault`를 사용합니다. 따라서 Mac과 N100에서 같은 `.env` 설정을 사용할 수 있으며, 필요할 때만 별도 Vault 절대경로로 덮어씁니다.
...
개발 PC에서 `main`으로 push하면 상시 서비스만 자동 재배포되며, Windows 작업 스케줄러가 실행하는 `investing-crawler` 일회성 작업은 자동배포 대상에서 제외됩니다.
...
- `BACKUP_RETENTION_DAYS`, `SECURITY_LOG_RETENTION_DAYS`, `OBSIDIAN_NEWS_RETENTION_DAYS`로 오래된 데이터를 정리합니다.
