...
기능 브랜치에서 변경하고 테스트와 검토를 거쳐 PR을 병합합니다. 병합 후에는 CI 결과와 배포 분류를 확인합니다. Compose 안전 배포는 허용된 서비스만 대상으로 하며, K3s에서 실행 중인 서비스는 제외합니다. K3s 앱 이미지는 별도 교체 절차를 사용합니다.

코드가 병합됐다는 사실과 운영 서버에 적용됐다는 사실은 구분합니다. 예를 들어 뉴스 알림의 영속 상태 관리 보완은 저장소에 반영됐지만, 이전 버전으로 복구할 때의 호환성 보완이 필요해 운영 이미지에는 아직 적용하지 않았습니다.
...
현재 남은 작업은 뉴스 알림 보완의 운영 적용과 SLO 증적 수집 오류 수정입니다. SLO 수정안은 로컬 검증을 마쳤지만 운영 수집기에는 적용하지 않았습니다. 코드·배포 상태와 검증 이력은 [개발 계획](docs/20260921_프로젝트보완_개발계획.md)과 [앱 배포 검증 결과](docs/reviews/20260921_K3s앱배포_검증결과.md)에 기록하고 있습니다.
