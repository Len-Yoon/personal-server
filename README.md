...
`car-care-worker`는 공개 웹 서비스가 아니며 Caddy 또는 외부 인바운드 경로를 사용하지 않습니다. `.env`에는 `CAR_CARE_TELEGRAM_BOT_TOKEN`, `CAR_CARE_TELEGRAM_CHAT_ID`를 설정해야 합니다. SQLite 상태는 `data/car-care`에만 저장됩니다.

지원 명령은 `/차량`, `/주행거리 <km>`, `/정비완료 엔진오일 [km]`, `/정비완료 미션오일 [km]`, `/정비목록`, `/알림테스트`입니다. 최초 정비 이력은 `/정비완료` 명령으로 수동 등록합니다.

Hyundai 연동은 선택 사항입니다. 실제 API 승인과 차량 응답 필드가 확인된 경우에만 `HYUNDAI_*` 값을 `.env`에 설정합니다. 미설정 시 수동 모드로 동작합니다.
