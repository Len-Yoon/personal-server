...
`car-care-worker`는 Telegram long polling으로 동작합니다. Hyundai OAuth 완료를 위해 `car.len.pe.kr`의 콜백 경로만 Cloudflare Tunnel에서 로컬 `8015` 포트로 전달하며, Caddy 경로는 사용하지 않습니다.
