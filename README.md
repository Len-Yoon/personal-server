...
`portfolio.len.pe.kr`은 공개 경력 포트폴리오이며, 기존 `len.pe.kr` 개인 서버 홈과 분리되어 있습니다.

- 공개 주소: `https://portfolio.len.pe.kr`
- 편집 주소: `https://portfolio.len.pe.kr/admin`
- 편집 비밀번호: `.env`의 `PORTFOLIO_ADMIN_PASSWORD`

관리자 화면에서 Markdown으로 경력과 개인 프로젝트를 작성해 저장하면 공개 페이지에 반영됩니다. Markdown의 원시 HTML은 허용하지 않습니다.
...
- `BACKUP_RETENTION_DAYS`, `SECURITY_LOG_RETENTION_DAYS`로 오래된 데이터를 정리합니다.
