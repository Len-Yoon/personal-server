...
docker buildx build --platform linux/amd64 --tag "$image" --file "$dockerfile" --output type=oci,dest="$output",annotation-manifest-descriptor.io.personal-server.image-ref="$image" "$app"
