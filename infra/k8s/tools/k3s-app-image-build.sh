...
docker buildx build --platform linux/amd64 --tag "$image" --file "$dockerfile" --output type=oci,dest="$output",annotation-manifest-descriptor.org.opencontainers.image.ref.name="$image" "$app"
