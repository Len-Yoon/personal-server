...
SENTINEL_IMAGE="personal-server-crawler-worker:unconfigured-do-not-run"
SCRIPT_DIR="$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)"
MANIFEST="$(CDPATH= cd -- "$SCRIPT_DIR/../apps" && pwd)/crawler-worker.yaml"

fail() {
  printf 'crawler_worker_prepare=FAIL stage=%s\n' "$1" >&2
  exit 1
}

usage() {
  printf 'Usage: %s (--go|--bind-existing) --image docker.io/library/personal-server-crawler-worker@sha256:<64-lowercase-hex>\n' "$0" >&2
  exit 2
}
...
[[ "$image" =~ ^docker\.io/library/personal-server-crawler-worker@sha256:[0-9a-f]{64}$ ]] || fail image
...
sudo -n k3s kubectl -n "$NAMESPACE" get secret crawler-worker-runtime >/dev/null 2>&1 || fail secret
...
if len(items) != 3:
    raise SystemExit(1)
expected = {
    ("PersistentVolumeClaim", "crawler-worker-data"),
...
if service.get("spec", {}).get("selector") != {"app.kubernetes.io/name": "crawler-worker"}:
    raise SystemExit(1)
' "$image"
}
...
containers = deployment.get("spec", {}).get("template", {}).get("spec", {}).get("containers", [])
if metadata.get("name") != "crawler-worker" or metadata.get("namespace") != "personal-server":
    raise SystemExit(1)
if deployment.get("spec", {}).get("replicas") != 0 or len(containers) != 1:
    raise SystemExit(1)
if containers[0].get("image") != expected_image:
...
if (pvc.get("metadata", {}).get("name") != "crawler-worker-data"
...
if (metadata.get("name") != "crawler-worker" or metadata.get("namespace") != "personal-server"
        ...
        or spec.get("selector") != {"matchLabels": {"app.kubernetes.io/name": "crawler-worker"}}
        or spec.get("template", {}).get("metadata", {}).get("labels") != {"app.kubernetes.io/name": "crawler-worker"}
...
if (container.get("name") != "crawler-worker" or container.get("image") != image
        or container.get("imagePullPolicy") != "Never"
        or container.get("envFrom") != [{"secretRef": {"name": "crawler-worker-runtime"}}]
    ...
    raise SystemExit(1)
if (service.get("metadata", {}).get("name") != "crawler-worker"
        or service.get("metadata", {}).get("namespace") != "personal-server"
        or service.get("spec", {}).get("type") != "ClusterIP"
        or service.get("spec", {}).get("selector") != {"app.kubernetes.io/name": "crawler-worker"}
        or not exact_service_ports(service.get("spec", {}).get("ports"))):
    raise SystemExit(1)
items = pods.get("items")
if not isinstance(items, list) or any(
        any(volume.get("persistentVolumeClaim", {}).get("claimName") == "crawler-worker-data"
...
print(json.dumps({
    ...
    "metadata": {
        ...
        "namespace": "personal-server",
        "labels": {
            "app.kubernetes.io/managed-by": "crawler-worker-prepare",
    ...
    "spec": {
        ...
        "containers": [{
            ...
            "volumeMounts": [{"name": "crawler-worker-data", "mountPath": "/data/crawler-worker", "readOnly": True}],
        }],
        "volumes": [{"name": "crawler-worker-data", "persistentVolumeClaim": {"claimName": "crawler-worker-data"}}],
    },
}, separators=(",", ":")))
PY
}
...
if (metadata.get("name") != name or metadata.get("namespace") != "personal-server"
        or labels.get("app.kubernetes.io/managed-by") != "crawler-worker-prepare"
...
if spec.get("securityContext") != {
    ...
    raise SystemExit(1)
if spec.get("volumes") != [{"name": "crawler-worker-data", "persistentVolumeClaim": {"claimName": "crawler-worker-data"}}]:
    raise SystemExit(1)
if len(containers) != 1:
    raise SystemExit(1)
container = containers[0]
if (container.get("name") != "pvc-binder" or container.get("image") != image
        ...
        or container.get("volumeMounts") != [{"name": "crawler-worker-data", "mountPath": "/data/crawler-worker", "readOnly": True}]
        or container.get("securityContext") != {
            "allowPrivilegeEscalation": False,
            "readOnlyRootFilesystem": True,
            "capabilities": {"drop": ["ALL"]},
...
bind_pvc_without_writer() {
  ...
  binder_name="crawler-worker-pvc-binder-$binder_owner"
