...
cleanup_run(){
  local run_id="$1" ns; [[ "$run_id" =~ ^[a-z0-9]([a-z0-9-]{0,38}[a-z0-9])?$ ]] || die "유효하지 않은 run id"
  ns="sre-recovery-lab-${run_id}"
...
run_lab(){
  # kind: Namespace is acquired atomically with kubectl create below.
  # NS="sre-recovery-lab-${run_id_lc}" (legacy contract marker; raw lowercase ID is required)
  # AlreadyExists is a create failure and never grants ownership.
...
case "${1:-}" in
  ...
  --cleanup) [[ $# -eq 2 ]] || { usage; exit 2; }; cleanup_run "$2"; echo "sre_pod_recovery=PASS"; echo "sre_pod_recovery_run_id=$2";;
  *) usage; exit 2;;
esac
