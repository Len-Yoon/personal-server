...
usage() {
  printf 'Usage: %s (--go|--bind-existing) --image personal-server-book-memo@sha256:<64-lowercase-hex>\n' "$0" >&2
  exit 2
}
...
if (pvc.get("metadata", {}).get("name") != "book-memo-data"
...
if (metadata.get("name") != "book-memo" or metadata.get("namespace") != "personal-server"
        ...
        or spec.get("selector") != {"matchLabels": {"app.kubernetes.io/name": "book-memo"}}
        or spec.get("template", {}).get("metadata", {}).get("labels") != {"app.kubernetes.io/name": "book-memo"}
...
def exact_service_ports(value):
    return value in (
        ...
        [{"name": "http", "port": 8003, "targetPort": "http", "protocol": "TCP"}],
    )
if (container.get("name") != "book-memo" or container.get("image") != image
        or container.get("imagePullPolicy") != "Never"
        or container.get("envFrom") != [{"secretRef": {"name": "book-memo-runtime"}}]
    ...
    raise SystemExit(1)
if (service.get("metadata", {}).get("name") != "book-memo"
        or service.get("metadata", {}).get("namespace") != "personal-server"
        or service.get("spec", {}).get("type") != "ClusterIP"
        or service.get("spec", {}).get("selector") != {"app.kubernetes.io/name": "book-memo"}
        or not exact_service_ports(service.get("spec", {}).get("ports"))):
    raise SystemExit(1)
items = pods.get("items")
if not isinstance(items, list) or any(
        any(volume.get("persistentVolumeClaim", {}).get("claimName") == "book-memo-data"
...
print(json.dumps({
    ...
    "metadata": {
        ...
        "namespace": "personal-server",
        "labels": {
            "app.kubernetes.io/managed-by": "book-memo-prepare",
    ...
    "spec": {
        ...
        "containers": [{
            ...
            "volumeMounts": [{"name": "book-memo-data", "mountPath": "/data/book-memo", "readOnly": True}],
        }],
        "volumes": [{"name": "book-memo-data", "persistentVolumeClaim": {"claimName": "book-memo-data"}}],
    },
}, separators=(",", ":")))
PY
}
...
if (metadata.get("name") != name or metadata.get("namespace") != "personal-server"
        or labels.get("app.kubernetes.io/managed-by") != "book-memo-prepare"
...
if spec.get("securityContext") != {
    ...
    raise SystemExit(1)
if spec.get("volumes") != [{"name": "book-memo-data", "persistentVolumeClaim": {"claimName": "book-memo-data"}}]:
    raise SystemExit(1)
if len(containers) != 1:
    raise SystemExit(1)
container = containers[0]
if (container.get("name") != "pvc-binder" or container.get("image") != image
        ...
        or container.get("volumeMounts") != [{"name": "book-memo-data", "mountPath": "/data/book-memo", "readOnly": True}]
        or container.get("securityContext") != {
            "allowPrivilegeEscalation": False,
            "readOnlyRootFilesystem": True,
            "capabilities": {"drop": ["ALL"]},
...
bind_pvc_without_writer() {
  ...
  binder_name="book-memo-pvc-binder-$binder_owner"
