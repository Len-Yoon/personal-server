...
HELPER = "youtube-memo-cutover-data"
MOUNT = "/data/youtube-memo"
...
def require_owned_helper():
    ...
    require(metadata.get("uid") == helper_uid)
...
try:
    ...
    image = options["--image"]
    require(re.fullmatch(r"docker\.io/library/personal-server-youtube-memo@sha256:[0-9a-f]{64}", image))
    source = Path(options["--source"])
    require(source.is_absolute() and source.is_dir() and source.resolve() == source
            and source != Path("/"))
    database = options["--database"]
    require(database == "youtube_memo.sqlite3")
    ...
    require(container.get("envFrom") == [{"secretRef": {"name": "youtube-memo-runtime"}}]
    ...
    if mode == "--prepare":
        stage = "dry_run"
        manifest = Path(sys.argv[1]).resolve().parents[1] / "apps" / "youtube-memo.yaml"
        ...
        run(["docker", "start", APP])
        wait_healthy()
    print("youtube_memo_cutover=PASS")
except Exception:
    recover()
    print("youtube_memo_cutover=FAIL stage=" + stage, file=sys.stderr)
    sys.exit(1)
PY
