...
MANIFEST = ROOT / "infra" / "k8s" / "apps" / "crawler-worker.yaml"
CUTOVER = ROOT / "infra" / "k8s" / "tools" / "crawler-worker-cutover.sh"
PREPARE = ROOT / "infra" / "k8s" / "tools" / "crawler-worker-prepare.sh"
UNCONFIGURED_IMAGE = "personal-server-crawler-worker:unconfigured-do-not-run"


class CrawlerWorkerCutoverTests(unittest.TestCase):
    ...
    def run_cutover(self, *modes, compose_state="exited", scenario="ok", image=None,
        ...
        for item in (source, target, binaries):
            item.mkdir()
        with sqlite3.connect(source / "news_summaries.sqlite3") as connection:
            connection.execute("create table memo (text text)")
            connection.execute("insert into memo values ('private-fixture')")
        (source / "news_archive.json").write_text('{"telegram_notifications_initialized": true}\n')
        (source / "news_collection_status.json").write_text('{"consecutive_failures": 0}\n')
        if scenario == "no_sqlite":
            (source / "news_summaries.sqlite3").unlink()
        if scenario == "missing_archive":
            (source / "news_archive.json").unlink()
        if scenario == "missing_status":
            (source / "news_collection_status.json").unlink()
        if scenario == "invalid_archive":
            (source / "news_archive.json").write_text("not json")
        if scenario == "bad_sqlite":
            (source / "news_summaries.sqlite3").write_text("not a database")
        if scenario == "symlink":
            (source / "link").symlink_to(source / "news_summaries.sqlite3")
        ...
        if modes == ("--rollback",):
            ...
            if scenario == "changed_data":
                with sqlite3.connect(target / "news_summaries.sqlite3") as connection:
                    connection.execute("insert into memo values ('new-data')")
            if scenario == "changed_news_state":
                (target / "news_archive.json").write_text('{"telegram_notifications_initialized": false}')
        (base / "state").write_text(json.dumps(state))
        chosen_image = image or "docker.io/library/personal-server-crawler-worker@sha256:" + "a" * 64
        ...
        if scenario == "inline_env":
            deployed["spec"]["template"]["spec"]["containers"][0]["env"] = [
                {"name": "NEWS_DB_PATH", "value": "/tmp/override.sqlite3"},
            ]
...
def fail():
    ...
    sys.exit(1)
def mutate_helper(payload, phase):
    prefix = 'helper_' + phase + '_'
    if not scenario.startswith(prefix): return payload
    mutation = scenario[len(prefix):]
    pod = payload['spec']
    container = pod['containers'][0]
    if mutation == 'init': pod['initContainers'] = [{'name': 'writer', 'image': fixture['image']}]
    elif mutation == 'ephemeral': pod['ephemeralContainers'] = [{'name': 'writer', 'image': fixture['image']}]
    elif mutation == 'sidecar': pod['containers'].append({'name': 'writer', 'image': fixture['image']})
    elif mutation == 'lifecycle': container['lifecycle'] = {'postStart': {'exec': {'command': ['true']}}}
    elif mutation == 'env': container['env'] = [{'name': 'PYTHONSTARTUP', 'value': '/tmp/writer.py'}]
    elif mutation == 'env_from': container['envFrom'] = [{'secretRef': {'name': 'crawler-worker-runtime'}}]
    elif mutation == 'foreign_mount': container['volumeMounts'].append({'name': 'foreign', 'mountPath': '/foreign'})
    elif mutation == 'foreign_volume': pod['volumes'].append({'name': 'foreign', 'hostPath': {'path': '/foreign'}})
    elif mutation == 'command': container['command'] = ['uvicorn', 'app.main:app']
    elif mutation == 'image': container['image'] = 'unexpected:latest'
    elif mutation == 'security': container['securityContext']['allowPrivilegeEscalation'] = True
    elif mutation == 'pod_security': pod['securityContext']['runAsUser'] = 0
    elif mutation == 'args': container['args'] = ['write']
    elif mutation in {'readinessProbe', 'livenessProbe', 'startupProbe'}:
        container[mutation] = {'exec': {'command': ['true']}}
    else: fail()
    return payload
if name == 'docker':
    if args[:2] == ['inspect', '--format']:
        if args[2] == '{{json .State}}':
            ...
            output([{'Type': 'bind', 'Source': fixture['source'], 'Destination': '/data/crawler-worker', 'RW': True}])
        else: fail()
    elif args[:1] == ['stop']:
        ...
        if scenario in ('rollback_divergence', 'rollback_unverifiable'):
            source = pathlib.Path(fixture['source']) / 'news_summaries.sqlite3'
            ...
            fail()
    elif args[:2] == ['exec', 'homeops-executor']:
        if scenario == 'recovery_guard_missing': fail()
        print('allowed' if scenario == 'recovery_allows_crawler'
              or (scenario == 'recovery_reenabled_during_rollout' and state['replicas']) else 'excluded')
    else: fail()
elif name == 'sudo':
    if args[:3] == ['-n', 'k3s', 'ctr']:
        ...
        if scenario != 'missing_image':
            ...
            listed_image = ('docker.io/library/personal-server-crawler-worker:k3s-test'
        ...
        if command[:2] == ['get', 'deployment']:
            ...
            output({'metadata': {'name': 'crawler-worker', 'namespace': 'personal-server'},
                    'spec': {'type': 'ClusterIP',
                             'selector': {'app.kubernetes.io/name': 'crawler-worker'},
                             'ports': [{'name': 'http', 'port': 8001, 'targetPort': 'http'}]}})
        elif command[:2] == ['get', 'pods']:
            pods = []
            if state['replicas'] or scenario == 'foreign_writer':
                pods.append({'metadata':{'name':'crawler-worker-writer'}, 'spec': {
                    'volumes':[{'persistentVolumeClaim':{'claimName':'crawler-worker-data'}}]}})
            if state['helper']:
                pods.append({'metadata':{'name':'crawler-worker-cutover-data'}, 'spec': {
                    'volumes':[] if scenario == 'helper_collision' else [{'persistentVolumeClaim':{'claimName':'crawler-worker-data'}}]}})
            output({'items':pods})
        elif command[:2] == ['get', 'pod']:
            if state['helper']:
                if scenario == 'helper_collision':
                    output({'metadata': {'name': 'crawler-worker-cutover-data', 'uid': 'existing-uid'}})
                else:
                    ...
                    helper = mutate_helper(helper, 'queried')
                    output(helper)
        elif command[:2] == ['get', 'pvc']:
            output({'metadata':{'name':'crawler-worker-data'}, 'status':{'phase': 'Pending' if scenario == 'pending_pvc' else 'Bound'},
                    'spec':{'accessModes':['ReadWriteOnce']}})
        elif command[:2] == ['apply', '--dry-run=server']:
            pass
        elif command[:2] == ['create', '--dry-run=server']:
            payload = json.load(sys.stdin)
            output(mutate_helper(payload, 'dryrun'))
            ...
            payload = mutate_helper(payload, 'created')
            ...
            child = [fixture['target'] if part == '/data/crawler-worker' else part for part in child]
            ...
            if scenario in ('docker_restarted_during_rollout', 'docker_restart_and_data_divergence'):
                state['compose'] = 'running'; save()
                if scenario == 'docker_restart_and_data_divergence':
                    (pathlib.Path(fixture['target']) / 'news_archive.json').write_text('{"new": true}')
            if scenario in ('rollout_divergence', 'rollout_unverifiable'):
                target = pathlib.Path(fixture['target']) / 'news_summaries.sqlite3'
...
'''
        ...
        result = subprocess.run(
            ["bash", str(CUTOVER), *modes, "--source", str(source), "--image", chosen_image,
    ...
    def test_go_requires_compose_writer_stopped_before_any_data_copy(self):
        ...
        self.assertIn("crawler_worker_cutover=FAIL", result.stderr)
        self.assertNotIn('"create"', calls.read_text())
        self.assertEqual(list(self.last_target.iterdir()), [])

    def test_go_refuses_active_or_unverifiable_homeops_recovery_before_stop(self):
        for scenario in ('recovery_allows_crawler', 'recovery_guard_missing'):
            with self.subTest(scenario=scenario):
                result, calls = self.run_cutover('--go', scenario=scenario)
                self.assertNotEqual(result.returncode, 0)
                self.assertIn('stage=recovery_guard', result.stderr)
                self.assertNotIn('"stop"', calls.read_text())
                self.assertEqual(self.last_state['compose'], 'running')

    def test_docker_reactivation_during_rollout_never_returns_success(self):
        result, calls = self.run_cutover('--go', scenario='docker_restarted_during_rollout')
        self.assertNotEqual(result.returncode, 0)
        self.assertNotIn('crawler_worker_cutover=PASS', result.stdout)
        self.assertFalse(self.last_state['compose'] == 'running' and self.last_state['replicas'] == 1)
        self.assertIn('--replicas=0', calls.read_text())

    def test_recovery_reenabled_before_success_is_rejected(self):
        result, _ = self.run_cutover('--go', scenario='recovery_reenabled_during_rollout')
        self.assertNotEqual(result.returncode, 0)
        self.assertIn('stage=writer_confirmation', result.stderr)
        self.assertEqual(self.last_state, {'compose': 'running', 'replicas': 0, 'helper': False})

    def test_docker_reactivation_with_divergence_preserves_both_copies_and_stops_writers(self):
        result, calls = self.run_cutover('--go', scenario='docker_restart_and_data_divergence')
        self.assertNotEqual(result.returncode, 0)
        self.assertIn('stage=recovery_data_divergence', result.stderr)
        self.assertEqual(self.last_state, {'compose': 'exited', 'replicas': 0, 'helper': False})
        self.assertNotIn('"start"', calls.read_text())
        self.assertEqual(json.loads((self.last_target / 'news_archive.json').read_text()), {'new': True})

    def test_n100_executor_mounts_runtime_directory_readonly_for_atomic_replacements(self):
        class OverrideLoader(yaml.SafeLoader):
            pass
        OverrideLoader.add_constructor('!override', lambda loader, node: loader.construct_sequence(node, deep=True))
        config = yaml.load((ROOT / 'docker-compose.n100.yml').read_text(), Loader=OverrideLoader)
        executor = config['services']['homeops-executor']
        self.assertIn('HOMEOPS_RUNTIME_STATE_PATH=/run/personal-server/k3s-runtime-services.state', executor['environment'])
        self.assertEqual(executor['volumes'], [{
            'type': 'bind', 'source': '/var/lib/personal-server', 'target': '/run/personal-server',
    ...
    def test_legacy_database_argument_is_rejected_without_external_calls(self):
        result, calls = self.run_cutover("--check", database="other.sqlite3")
        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(calls.read_text(), "")

    def test_json_only_crawler_copies_archive_and_collection_state_without_sqlite(self):
        result, _ = self.run_cutover("--go", scenario="no_sqlite")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(self.last_state, {"compose": "exited", "replicas": 1, "helper": False})
        self.assertEqual({entry.name for entry in self.last_target.iterdir()},
                         {"news_archive.json", "news_collection_status.json"})
        self.assertTrue(json.loads((self.last_target / "news_archive.json").read_text())["telegram_notifications_initialized"])

    def test_missing_crawler_state_blocks_docker_stop(self):
        for scenario in ("missing_archive", "missing_status"):
            with self.subTest(scenario=scenario):
                result, calls = self.run_cutover("--go", scenario=scenario)
                self.assertNotEqual(result.returncode, 0)
                self.assertIn("stage=source", result.stderr)
                self.assertNotIn('"stop"', calls.read_text())

    def test_rollback_refuses_diverged_news_and_notification_state(self):
        result, calls = self.run_cutover("--rollback", scenario="changed_news_state")
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("stage=rollback_data", result.stderr)
        self.assertIn("--replicas=0", calls.read_text())
        self.assertIn("--replicas=1", calls.read_text())
        self.assertNotIn('"start"', calls.read_text())
        self.assertEqual(self.last_state, {"compose": "exited", "replicas": 1, "helper": False})

    def test_invalid_news_archive_prevents_k3s_start(self):
        result, calls = self.run_cutover("--go", scenario="invalid_archive")
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("stage=source_data", result.stderr)
        self.assertNotIn("--replicas=1", calls.read_text())
        self.assertEqual(self.last_state["replicas"], 0)
    ...
    def test_mutable_or_foreign_image_is_rejected_without_external_calls(self):
        for image in ("personal-server-crawler-worker:latest", "portal-web@sha256:" + "a"*64):
    ...
    def test_go_verifies_data_and_hands_over_one_writer(self):
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("crawler_worker_cutover=PASS", result.stdout)
        self.assertEqual(self.last_state, {"compose": "exited", "replicas": 1, "helper": False})
        with sqlite3.connect(self.last_target / "news_summaries.sqlite3") as connection:
        ...
        for name in ("news_archive.json", "news_collection_status.json"):
            self.assertEqual((self.last_target / name).read_bytes(), (self.last_target.parent / "source" / name).read_bytes())
        commands = [json.loads(line) for line in calls.read_text().splitlines()]
        stop = next(i for i, c in enumerate(commands) if c[:2] == ["docker", "stop"])
        copy = next(i for i, c in enumerate(commands) if c[-1] == "extract")
    ...
    def test_failed_go_restores_compose_and_stops_k3s(self):
        for scenario in ("bad_sqlite", "copy_failure", "digest_mismatch", "rollout_failure", "occupied_pvc"):
            with self.subTest(scenario=scenario):
                ...
                self.assertNotEqual(result.returncode, 0)
                self.assertRegex(result.stderr, r"^crawler_worker_cutover=FAIL stage=[a-z_]+\n$")
                self.assertEqual(self.last_state, {"compose": "running", "replicas": 0, "helper": False})
                if scenario == "occupied_pvc":
    ...
    def test_foreign_helper_replacement_blocks_copy_and_k3s_start(self):
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertNotIn('"personal-server", "exec"', calls.read_text())
        self.assertNotIn('--replicas=1', calls.read_text())
        self.assertEqual(self.last_state, {"compose": "exited", "replicas": 0, "helper": True})

    def test_helper_dry_run_rejects_writer_hooks_before_real_create(self):
        for mutation in ("init", "ephemeral", "sidecar", "lifecycle", "env", "env_from",
                         "foreign_mount", "foreign_volume", "command", "image", "security",
                         "pod_security", "args", "readinessProbe", "livenessProbe", "startupProbe"):
            with self.subTest(mutation=mutation):
                result, calls = self.run_cutover("--go", scenario="helper_dryrun_" + mutation)
                self.assertNotEqual(result.returncode, 0)
                commands = [json.loads(line) for line in calls.read_text().splitlines()]
                self.assertTrue(any("--dry-run=server" in command for command in commands))
                self.assertFalse(any("create" in command and "--dry-run=server" not in command
                                     for command in commands))
                self.assertNotIn('"personal-server", "exec"', calls.read_text())
                self.assertNotIn("--replicas=1", calls.read_text())
                self.assertFalse(self.last_state["helper"])

    def test_helper_actual_create_rejects_new_writer_hooks_before_data_access(self):
        for mutation in ("init", "lifecycle", "foreign_mount"):
            with self.subTest(mutation=mutation):
                result, calls = self.run_cutover("--go", scenario="helper_created_" + mutation)
                self.assertNotEqual(result.returncode, 0)
                self.assertIn("--dry-run=server", calls.read_text())
                self.assertNotIn('"personal-server", "exec"', calls.read_text())
                self.assertNotIn("--replicas=1", calls.read_text())
                self.assertFalse(self.last_state["helper"])
                self.assertIn("--raw=", calls.read_text())

    def test_helper_requery_rejects_mutation_before_data_access(self):
        for mutation in ("sidecar", "env", "foreign_volume"):
            with self.subTest(mutation=mutation):
                result, calls = self.run_cutover("--go", scenario="helper_queried_" + mutation)
                self.assertNotEqual(result.returncode, 0)
                self.assertNotIn('"personal-server", "exec"', calls.read_text())
                self.assertNotIn("--replicas=1", calls.read_text())
                self.assertFalse(self.last_state["helper"])
    ...
    def test_failed_rollback_start_never_reactivates_stale_or_unverifiable_pvc(self):
        for scenario, stage in (("rollback_divergence", "recovery_data_divergence"),
            ...
            with self.subTest(scenario=scenario):
                ...
                self.assertNotEqual(result.returncode, 0)
                self.assertEqual(result.stderr, f"crawler_worker_cutover=FAIL stage={stage}\n")
                self.assertEqual(self.last_state, {"compose": "exited", "replicas": 0, "helper": False})
                self.assertNotIn('--replicas=1', calls.read_text())
                with sqlite3.connect(self.last_target / "news_summaries.sqlite3") as connection:
                    self.assertEqual(connection.execute("select count(*) from memo").fetchone()[0], 1)
                if scenario == "rollback_divergence":
                    with sqlite3.connect(self.last_target.parent / "source" / "news_summaries.sqlite3") as connection:
    ...
    def test_replaced_helper_uid_is_never_deleted(self):
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(result.stderr, "crawler_worker_cutover=FAIL stage=recovery_required\n")
        self.assertEqual(self.last_state, {"compose": "exited", "replicas": 0, "helper": True})


class CrawlerWorkerPrepareTests(unittest.TestCase):
    def run_prepare(self, *arguments, secret_exists=True, listed_image=None,
        ...
        image = "docker.io/library/personal-server-crawler-worker@sha256:" + "a" * 64
...
def pvc_document():
    return {"kind": "PersistentVolumeClaim", "metadata": {"name": "crawler-worker-data", "namespace": "personal-server"},
            "spec": {"accessModes": ["ReadWriteOnce"], "resources": {"requests": {"storage": "1Gi"}}},
            "status": {"phase": "Bound" if fixture["pvc_bound"] else "Pending"}}
def deployment_document():
    document = {"kind": "Deployment", "metadata": {"name": "crawler-worker", "namespace": "personal-server", "labels": {"app.kubernetes.io/name": "crawler-worker"}},
      "spec": {"replicas": 0, "strategy": {"type": "Recreate"}, "selector": {"matchLabels": {"app.kubernetes.io/name": "crawler-worker"}},
      "template": {"metadata": {"labels": {"app.kubernetes.io/name": "crawler-worker"}}, "spec": {
      ...
      "containers": [{"name": "crawler-worker", "image": fixture["image"], "imagePullPolicy": "Never",
        "envFrom": [{"secretRef": {"name": "crawler-worker-runtime"}}],
        "securityContext": {"allowPrivilegeEscalation": False, "readOnlyRootFilesystem": True, "capabilities": {"drop": ["ALL"]}},
        "readinessProbe": {"httpGet": {"path": "/health", "port": "http"}},
        "livenessProbe": {"httpGet": {"path": "/health", "port": "http"}},
        "volumeMounts": [{"name": "crawler-worker-data", "mountPath": "/data/crawler-worker"}, {"name": "tmp", "mountPath": "/tmp"}]}],
      "volumes": [{"name": "crawler-worker-data", "persistentVolumeClaim": {"claimName": "crawler-worker-data"}}, {"name": "tmp", "emptyDir": {"medium": "Memory", "sizeLimit": "64Mi"}}]}}}}
    if fixture["existing_mutation"] == "wrong_image":
        document["spec"]["template"]["spec"]["containers"][0]["image"] = "personal-server-crawler-worker:mutable"
...
def service_document():
    document = {"kind": "Service", "metadata": {"name": "crawler-worker", "namespace": "personal-server"},
                "spec": {"type": "ClusterIP", "selector": {"app.kubernetes.io/name": "crawler-worker"},
                         "ports": [{"name": "http", "port": 8001, "targetPort": "http"}]}}
...
if command[:1] == ["get"] and command[1] in {"pvc", "deployment", "service"}:
    ...
    if fixture["applied"] and command[-2:] == ["-o", "name"]:
        print(resource + "/" + ("crawler-worker-data" if resource == "pvc" else "crawler-worker"))
    ...
    if command[1] == "service" and fixture["foreign_resource_present"]:
        print("service/crawler-worker")
    sys.exit(0)
if command[:2] == ["get", "pods"]:
    pods = []
    if fixture["existing_mutation"] == "writer_pod":
        pods.append({"metadata": {"name": "foreign-writer"}, "spec": {"volumes": [{"persistentVolumeClaim": {"claimName": "crawler-worker-data"}}]}})
...
if command[:2] == ["create", "--dry-run=server"]:
    ...
    if "unconfigured-do-not-run" in rendered or fixture["image"] not in rendered or "replicas: 0" not in rendered:
        sys.exit(1)
    pvc = {"kind": "PersistentVolumeClaim", "metadata": {"name": "crawler-worker-data", "namespace": "personal-server"}, "spec": {"accessModes": ["ReadWriteOnce"]}}
    deployment = {"kind": "Deployment", "metadata": {"name": "crawler-worker", "namespace": "personal-server"}, "spec": {"replicas": 0, "template": {"spec": {"containers": [{"image": fixture["image"], "volumeMounts": [{"name": "crawler-worker-data", "mountPath": "/data/crawler-worker"}, {"name": "tmp", "mountPath": "/tmp"}]}], "volumes": [{"name": "crawler-worker-data", "persistentVolumeClaim": {"claimName": "crawler-worker-data"}}, {"name": "tmp", "emptyDir": {"medium": "Memory", "sizeLimit": "64Mi"}}]}}}}
    service = {"kind": "Service", "metadata": {"name": "crawler-worker", "namespace": "personal-server"}, "spec": {"selector": {"app.kubernetes.io/name": "crawler-worker"}}}
...
'''
    ...
    def test_prepare_applies_only_rendered_replica_zero_resources(self):
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stdout, "crawler_worker_prepare=PASS\n")
    ...
    def test_prepare_binds_pending_pvc_with_owned_nonwriter_binder_then_removes_it(self):
        ...
        self.assertRegex(binder["metadata"]["name"], r"^crawler-worker-pvc-binder-[0-9a-f]{32}$")
        labels = binder["metadata"]["labels"]
        self.assertEqual(labels["app.kubernetes.io/managed-by"], "crawler-worker-prepare")
        ...
        self.assertEqual(pod["volumes"], [{"name": "crawler-worker-data", "persistentVolumeClaim": {"claimName": "crawler-worker-data"}}])
        ...
        self.assertEqual(container["volumeMounts"], [{
            "name": "crawler-worker-data", "mountPath": "/data/crawler-worker", "readOnly": True,
        }])
    ...
    def test_prepare_binder_failure_never_deletes_base_resources_or_foreign_binder(self):
        ...
        for scenario in ("binder_not_ready", "binder_unowned", "binder_replaced"):
            with self.subTest(scenario=scenario):
                ...
                self.assertNotEqual(result.returncode, 0)
                self.assertRegex(result.stderr, r"^crawler_worker_prepare=FAIL stage=(pvc_bind|recovery)\n$")
                self.assertTrue(self.last_prepare_fixture["applied"])
    ...
    def test_prepare_rejects_server_dry_run_binder_writer_hooks_before_actual_create(self):
        ...
        for scenario in (
            ...
            with self.subTest(scenario=scenario):
                ...
                self.assertNotEqual(result.returncode, 0)
                self.assertEqual(result.stderr, "crawler_worker_prepare=FAIL stage=binder_contract\n")
                commands = [json.loads(line) for line in calls.splitlines()]
                self.assertTrue(any("--dry-run=server" in command for command in commands))
    ...
    def test_prepare_uncertain_binder_create_recovers_only_verified_owned_pod(self):
        ...
        for scenario, raw_delete_expected in (
            ...
            with self.subTest(scenario=scenario):
                result, calls = self.run_prepare("--go", "--image", image, binder_scenario=scenario)
                self.assertNotEqual(result.returncode, 0)
                self.assertEqual(result.stderr, "crawler_worker_prepare=FAIL stage=recovery\n")
    ...
    def test_bind_existing_only_binds_exact_inert_resources_without_persistent_create(self):
        ...
        commands = [json.loads(line) for line in calls.splitlines()]
        self.assertTrue(any(command[-5:] == ["get", "pvc", "crawler-worker-data", "-o", "json"] for command in commands))
        self.assertTrue(any(command[-5:] == ["get", "deployment", "crawler-worker", "-o", "json"] for command in commands))
        self.assertTrue(any(command[-5:] == ["get", "service", "crawler-worker", "-o", "json"] for command in commands))
    ...
    def test_bind_existing_rejects_missing_mutated_or_writer_resources_without_binder(self):
        ...
        for mutation in ("missing_deployment", "wrong_image", "bad_service", "writer_pod"):
            with self.subTest(mutation=mutation):
                ...
                self.assertNotEqual(result.returncode, 0)
                self.assertEqual(result.stderr, "crawler_worker_prepare=FAIL stage=existing\n")
                self.assertIsNone(self.last_prepare_fixture["binder"])
                self.assertNotIn("--raw=", calls)
    ...
    def test_prepare_rejects_combined_create_and_bind_existing_modes_without_external_calls(self):
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(result.stderr, "crawler_worker_prepare=FAIL stage=arguments\n")
    ...
    def test_prepare_missing_secret_stops_before_image_or_resource_changes(self):
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(result.stderr, "crawler_worker_prepare=FAIL stage=secret\n")
    ...
    def test_prepare_rejects_tag_only_listing_before_any_resource_create(self):
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(result.stderr, "crawler_worker_prepare=FAIL stage=image\n")
    ...
    def test_prepare_fails_closed_when_resource_appears_after_dry_run(self):
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(result.stderr, "crawler_worker_prepare=FAIL stage=create\n")
        self.assertIn('"create", "--dry-run=server"', calls)
        self.assertIn('"create", "-f", "-"', calls)
    ...
    def test_prepare_rejects_any_unexpected_rendered_resource_before_create(self):
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertEqual(result.stderr, "crawler_worker_prepare=FAIL stage=server_dry_run\n")
        self.assertIn('"create", "--dry-run=server"', calls)
    ...
    def test_prepare_rejects_crlf_manifest_without_exact_inert_contract(self):
        ...
        for mutation in ("bad_replica", "duplicate_sentinel"):
            with self.subTest(mutation=mutation):
                ...
                self.assertNotEqual(result.returncode, 0)
                self.assertEqual(result.stderr, "crawler_worker_prepare=FAIL stage=manifest\n")
                self.assertEqual(calls, "")


class CrawlerWorkerManifestTests(unittest.TestCase):
    def assert_manifest_contract(self, documents):
        ...
        self.assertEqual(pvc["metadata"], {"name": "crawler-worker-data", "namespace": "personal-server"})
        ...
        self.assertEqual(
            ...
            [
                {"name": "crawler-worker-data", "mountPath": "/data/crawler-worker"},
                {"name": "tmp", "mountPath": "/tmp"},
            ],
        )
        volumes = {volume["name"]: volume for volume in pod["volumes"]}
        self.assertEqual(
            volumes["crawler-worker-data"],
            {"name": "crawler-worker-data", "persistentVolumeClaim": {"claimName": "crawler-worker-data"}},
        )
        ...
        self.assertNotIn("env", container)
        self.assertEqual(container["envFrom"], [{"secretRef": {"name": "crawler-worker-runtime"}}])
        self.assertEqual(service["metadata"]["labels"]["app.kubernetes.io/name"], "crawler-worker")
        self.assertEqual(service["spec"]["type"], "ClusterIP")
        self.assertEqual(service["spec"]["ports"], [{"name": "http", "port": 8001, "targetPort": "http"}])
        self.assertNotIn("nodePort", yaml.safe_dump(service))

    def test_crawler_worker_manifest_has_single_nonroot_writer_and_clusterip_service(self):
    ...
    def test_crawler_worker_manifest_uses_its_pvc_and_hardened_local_image_contract(self):
