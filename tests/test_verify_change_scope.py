...
class VerifyChangeScopeTests(unittest.TestCase):
    ...
    def test_token_measurement_recorder_is_maintenance_policy_file(self):
        code, evidence = run_scope(
            "scripts/record_token_measurement.py", executed_checks=("maintenance",)
        )

        self.assertEqual(code, 0)
        self.assertEqual(
            evidence["automation_files"], ["scripts/record_token_measurement.py"]
        )
        self.assertEqual(evidence["required_checks"], ["maintenance"])
        self.assertEqual(evidence["blocked_files"], [])

    def test_local_test_venv_setup_is_a_maintenance_policy_file(self):
        code, evidence = run_scope(
            "scripts/setup-local-test-venvs.py", executed_checks=("maintenance",)
        )

        self.assertEqual(code, 0)
        self.assertEqual(evidence["automation_files"], ["scripts/setup-local-test-venvs.py"])
        self.assertEqual(evidence["required_checks"], ["maintenance"])
        self.assertEqual(evidence["blocked_files"], [])
    ...
    def test_safe_n100_deployment_scripts_are_narrow_maintenance_policy_files(self):
        paths = (
