...
class PortalHttpGrafanaDashboardTests(unittest.TestCase):
    def test_dashboard_exposes_portal_http_operational_panels(self):
        ...
        self.assertEqual(panels["5xx 오류 비율"]["targets"][0]["expr"], "sum(rate(portal_http_requests_total{status_code=~\"5..\"}[5m])) / clamp_min(sum(rate(portal_http_requests_total[5m])), 1)")
