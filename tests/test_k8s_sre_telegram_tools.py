...
class SreTelegramToolContractTest(unittest.TestCase):
    ...
    def test_install_accepts_containerd_canonical_image_reference_and_creates_resources(self):
        result, calls = self.run_tool(
            ...
            stubs={
                ...
                "case \"$1\" in\n"
                "  template) exit 0;;\n"
                "  status) printf '{\"info\":{\"status\":\"deployed\"},\"version\":2}\\n'; exit 0;;\n"
                "  upgrade) exit 0;;\n"
                "  *) exit 0;;\n"
                "esac\n",
            },
    ...
    def test_install_rolls_back_monitoring_relay_state_binding_after_later_create_failure(self):
        result, calls = self.run_tool(
            "sre-telegram-install.sh",
            "--apply",
            stubs={
                ...
                "  *describe*) printf 'telegram_bot_token: 3 bytes\\nallowed_chat_id: 2 bytes\\nalertmanager_auth_token: 3 bytes\\nalertmanager.yaml: 4 bytes\\n'; exit 0;;\n"
                "  *'apply --dry-run=client'*) exit 0;;\n"
                "  *'monitoring-RoleBinding-sre-telegram-relay-state.yaml'*) printf 'rolebinding.rbac.authorization.k8s.io/sre-telegram-relay-state created\\n'; exit 0;;\n"
                "  *'monitoring-Deployment-sre-telegram-relay.yaml'*) printf 'Error from server (InternalError): later resource create failed\\n'; exit 1;;\n"
                "  *'images list'*) printf 'REF TYPE DIGEST SIZE PLATFORMS LABELS\\npersonal-server-sre-telegram-relay:latest x sha256:abc 1MB linux/amd64 -\\n'; exit 0;;\n"
                "  *delete*) exit 0;;\n"
                "  *) exit 0;;\n"
        ...
        self.assertTrue(result.stdout.rstrip().endswith("sre_telegram_install=FAIL"))
        self.assertIn(
            "-n monitoring delete rolebinding.rbac.authorization.k8s.io/sre-telegram-relay-state",
            calls,
        )
    ...
    def test_failed_upgrade_rolls_back_prior_revision_and_requires_deployed_status(self):
        result, calls = self.run_tool(
            ...
            stubs={
                ...
                "    if [ \"$count\" -lt 1 ]; then printf '{\"info\":{\"status\":\"deployed\"},\"version\":2}\\n'; else printf '{\"info\":{\"status\":\"deployed\"},\"version\":3}\\n'; fi\n"
                "    printf 'helm %s\\n' \"$*\" >> \"$CALLS\"\n"
                "    exit 0;;\n"
                "  get)\n"
                "    printf 'helm %s\\n' \"$*\" >> \"$CALLS\"\n"
    ...
    def test_install_accepts_numeric_helm_status_version_as_previous_revision(self):
        result, calls = self.run_tool(
            ...
            stubs={
                ...
                "case \"$1\" in\n"
                "  template) exit 0;;\n"
                "  status) printf '{\"name\":\"personal-server-monitoring\",\"info\":{\"status\":\"deployed\"},\"chart\":{\"metadata\":{\"version\":\"88.6.1\"}},\"version\":3,\"namespace\":\"monitoring\"}\\n'; exit 0;;\n"
                "  upgrade) printf 'helm %s\\n' \"$*\" >> \"$CALLS\"; exit 1;;\n"
                "  rollback) printf 'helm %s\\n' \"$*\" >> \"$CALLS\"; exit 0;;\n"
                "  *) exit 0;;\n"
                "esac\n",
    ...
    def test_install_never_snapshots_helm_values_or_manifest(self):
        result, calls = self.run_tool(
            ...
            stubs={
                ...
                "  status)\n"
                "    count=$(grep -c '^helm status' \"$CALLS\" 2>/dev/null || true)\n"
                "    if [ \"$count\" -lt 1 ]; then printf '{\"info\":{\"status\":\"deployed\"},\"version\":2}\\n'; else printf '{\"info\":{\"status\":\"deployed\"},\"version\":4}\\n'; fi\n"
                "    printf 'helm %s\\n' \"$*\" >> \"$CALLS\"\n"
                "    exit 0;;\n"
                "  get)\n"
