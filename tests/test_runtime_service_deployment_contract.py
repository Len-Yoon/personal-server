...
class RuntimeServiceDeploymentContractTests(unittest.TestCase):
    def _run_health_check(self, runtime_state):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (fake_bin / "docker").write_text(
                ...
                f"printf 'docker %s\\n' \"$*\" >> '{calls}'\n"
                "if [ \"$1\" = compose ]; then\n"
                "  printf '%s\\n' system-agent crawler-worker youtube-memo car-care-worker caddy homeops-executor portal-web\n"
    ...
    def test_book_memo_k3s_health_uses_kubernetes_resources_not_docker_loopback(self):
        ...
        self.assertIn('"service/book-memo"', text)
        self.assertIn('"endpoints/book-memo"', text)
        self.assertIn('"pvc/book-memo-data"', text)
