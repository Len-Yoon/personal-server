...
- 단, 사용자가 2026-09-17에 명시 승인한 Book Memo 1회 K3s 공개 경로 전환에 한해, 이미 검증된 K3s Service ClusterIP로 `books.len.pe.kr`의 Cloudflare Tunnel ingress 한 항목만 원자적으로 변경하고 Tunnel 서비스를 재기동할 수 있음. 변경 전후 Books·Portal 외부 health를 각각 3회 확인하며, 실패 시 ingress 설정만 즉시 원복함. Docker writer 재시작, K3s writer·PVC·Secret·다른 ingress 변경은 금지함.
- 사용자가 2026-09-17에 명시 승인한 Book Memo Caddy source 정합성 보완에 한해 `caddy/Caddyfile`, `docker-compose.n100.yml`, `scripts/deploy-n100.sh`, `scripts/windows-bootstrap.sh`, `scripts/verify-n100-deployment-health.sh`와 직접 관련 테스트·문서를 최소 범위로 수정할 수 있음. K3s 상태에서는 Service·Endpoint·Deployment 준비 상태를 먼저 확인한 뒤 조회한 upstream만 Caddy 재생성 프로세스에 전달하며, Git에 ClusterIP를 고정하지 않음. 조회 실패 시 Caddy를 재생성하지 않고 중단함. 실제 N100 Caddy·Tunnel·K3s 리소스 적용은 별도 사용자 승인 전 수행하지 않음.
...
## N100 접속·실제 적용·반복 방지 규칙

- Git 커밋·push·PR 병합은 저장소 코드 반영일 뿐 N100 운영 적용이 아님. `적용 완료` 또는 `배포 완료`는 N100에서 최신 커밋 확인, 필요한 이미지 빌드·K3s 반입·매니페스트 적용, 실제 검증이 모두 끝난 경우에만 표현함.
- N100 운영 작업 승인 시 실제 적용 여부를 다음 상태로 분리해 즉시 보고함: `저장소 병합`, `N100 동기화`, `이미지 반입`, `수동 검증`, `자동 실행 활성화`, `외부 health 검증`. 완료하지 않은 단계를 완료처럼 요약하지 않음.
- N100 접속 정보나 키 관련 질문을 다시 하기 전에 현재 대화, 저장소 문서, 로컬 SSH 설정, 등록된 공개키 파일명과 SSH agent 상태를 읽기 전용으로 먼저 확인함. 기존 키를 삭제·재생성·복제·출력하지 않음.
- 내부 IP, SSH 개인키 경로, 계정명, 비밀번호, Secret 값은 Git·AGENTS.md·문서·로그에 저장하거나 출력하지 않음. 재사용 가능한 접속 별칭은 운영자가 로컬 SSH 설정에서 관리함.
- N100 원격 셸이 Windows PowerShell인 경우 Windows 명령과 WSL Bash 명령 경계를 명시적으로 분리함. Bash 전용 명령을 Windows 셸에 직접 전달하지 않으며, 외부 health는 WSL의 `curl` 또는 Windows `curl.exe`를 명시해 PowerShell 별칭 해석을 피함.
- 원격 적용 명령의 응답 수신이 실패하거나 불완전하면 같은 변경 명령을 즉시 재실행하지 않음. 먼저 N100의 커밋, 이미지, CronJob/Job 상태를 읽기 전용으로 확인해 실제 적용 여부를 판별한 뒤 다음 조치를 결정함.
- CronJob·이미지 태그처럼 같은 이름을 재사용하는 운영 변경은 수동 Job의 실제 성공(`portal_pvc_backup=PASS`)을 확인한 뒤에만 자동 실행을 활성화함. 실패 시 CronJob을 중지 상태로 유지하고 실패 단계만 보고함.
- 병합 후 Deploy N100이 실패하면 항상 실패 로그의 분류 값을 확인함. 허용 경로 밖 변경에 대한 `blocked_path`는 정책상 자동 배포 제외이며, 서비스 장애나 병합 실패로 표현하지 않음.
...
- 개발 작업은 시작, 구현 완료, 테스트 시작·종료, 독립 검토 요청·결과, 차단·대기, 커밋·PR·배포 직전의 단계가 바뀔 때마다 사용자에게 즉시 진행 상황을 보고함. 실제로 실행 중인 작업만 "진행 중"으로 표현하며, 멈춘 경우에는 멈춘 이유와 다음 조치를 즉시 알림. 사용자가 반복해서 상태를 묻지 않도록 장시간 작업 중에는 60초를 넘기지 않고 간단한 중간 상태를 공유함.
- 사용자 질문에 답하기 전에 현재 대화와 이미 확인한 운영 상태를 우선 사용함. 같은 IP·접속 경로·승인 범위·검증 결과를 다시 묻기 전에는 보유 정보와 안전한 읽기 전용 확인 수단을 먼저 점검하며, 정보가 없다는 결론은 실제 점검 근거와 함께만 보고함.
...
## 뉴스 PR #228 병합·workflow 처리 승인

- 사용자가 2026-09-21 뉴스 PR #228의 처리·병합·마무리를 명시 승인함. 해당 PR의 최신 main 통합, 검증, main 병합과 후속 workflow 실행·결과 확인은 같은 승인을 반복 요청하지 않고 진행함.
- 기존 금지 영역을 변경하거나 배포 분류를 우회하지 않음. 운영 상태의 읽기 전용 확인과 실제 workflow 결과를 근거로 저장소 반영·배포 제외·실제 적용 상태를 분리해 보고함.
